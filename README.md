...
- With semantic hierarchy enabled, every child tag independently generates a prefix and facet. Code validates the pair and stores the complete `prefix.facet` string. A prefix starts with the supported person's name, may contain any number of concrete descriptive words, and ends literally in `activity`, `plan`, `profile`, `possession`, or `relationship`. Any malformed tag receives exactly one error-aware repair attempt before the child window fails. No session prefix pool is generated or stored.
