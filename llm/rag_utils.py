...
from llm.embeddings import get_local_embedding, LOCAL_EMBEDDING_BATCH_SIZE


def get_embeddings(inputs, mode='context'):
    """Generate sentence/query vectors via a local sentence-transformers embedding model.
    ...
    with torch.no_grad():
        for i in tqdm(range(0, len(inputs), batch_size)):
            vecs = get_local_embedding(inputs[i:(i + batch_size)], batch_size=batch_size)
